...
void mastermonitor_task(void*p){
	...
	while(1)
	{
		//GHI 1 vào bit thứ 8 của slave modbus Coils(function code = 05)
		errorCode = eMBMasterReqWriteCoil(1,8,0xFF00, portMAX_DELAY);

		//đọc 8 bit đầu tiên của slave modbus DiscreaseInput(function code = 02)
		...
		//ghi vào 4 thanh ghi đầu tiên của slave modbus HoldingRegister (function code = 10)
		errorCode = eMBMasterReqWriteMultipleHoldingRegister(1,0,2,usModbusUserData,
																	portMAX_DELAY);
		vTaskDelay(2000);
...
#define PORT_MASTER_RTU		0

void masterpoll_task(void*p){

    eMBMasterInit(MB_RTU, PORT_MASTER_RTU, 115200,  MB_PAR_EVEN);
    eMBMasterEnable();
...
#define PORT_MODBUS_RTU		0	//don't care
...
void slavepoll_task(void*p){
	...
	eMBInit(MB_RTU, SLAVE_ADDRESS, PORT_MODBUS_RTU, 115200, MB_PAR_EVEN);
	eMBEnable();
	HAL_Delay(2);
